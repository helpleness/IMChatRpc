...
func (s *Chatserver) SendMessage(ctx context.Context, req *pb.SendMessageRequest) (*pb.SendMessageResponse, error) {
	...
	if status == "online" && !exists {
		err := s.storeMessageInQueue(ctx, message)
		if err != nil {
			return &pb.SendMessageResponse{
				Success:      false,
				ErrorMessage: "消息存储失败",
...
func (s *Chatserver) storeMessageInQueue(ctx context.Context, message *pb.MyMessage) error {
	queueName := "message_queue" // 队列名，可以按需设置
